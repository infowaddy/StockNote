...
namespace StockNote.WebAPI.Controllers
{
    ...
    public class StockInWarehouseController : ControllerBase
    {
        ...
        /// <returns></returns>
        [HttpPatch]
        [Route("{warehouseID:guid}/{itemID:guid}/{minimumThreshold:decimal}")]
        public async Task<IActionResult> UpdateMinimumThresholdOfStock( Guid warehouseID,  Guid itemID,  Decimal minimumThreshold)
        {
        ...
        /// <returns></returns>
        [HttpGet]
        [Route("{warehouseID:guid}/{itemID:guid}")]
        public async Task<IActionResult> GetStockInWarehouse(Guid warehouseID,  Guid itemID)
        {
        ...
        public async Task<IActionResult> GetAllStockInWarehouses(bool includeDeleted=false)
        {
            List<StockInWarehouse> stockInWarehouseList = await stockInWarehouseManager.GetStockInWarehouses(includeDeleted);
        ...
        public async Task<IActionResult> DeleteStockInWarehouse(Guid id, bool undelete = false)
        {
            bool result = await stockInWarehouseManager.ChangeActiveStatusOfStockInWarehouse(id, undelete);
            if (result)
                return StatusCode((int)HttpStatusCode.OK);
            else
                return StatusCode((int)HttpStatusCode.ExpectationFailed);
        ...
        [HttpDelete]
        [Route("{id:guid}")]
        public async Task<IActionResult> HardDeleteStockInWarehouse(Guid id)
        {
            bool result = await stockInWarehouseManager.DeleteStockInWarehouse(id);
            if (result)
                return StatusCode((int)HttpStatusCode.OK);
            else
                return StatusCode((int)HttpStatusCode.ExpectationFailed);
